...
**Execution:**
- **Arrange:** Mock the input to simulate the user entering choice '4' and integers where the divisor’s real and imaginary parts are both zero.
- **Act:** Call the `complex_arithmetic` function.
- **Assert:** Check that the function handles this gracefully, possibly by returning an error message or an appropriate default value.
...
def complex_arithmetic():
    ...
    if choice == "1":
        ...
        nums = list(map(int, input("Enter all numbers separated by space maximum 4 elements: ").split()))
        if len(nums) != 4 or (nums[2] == 0 and nums[3] == 0):
            return None
