...
class Test_MainComplexArithmetic:
    ...
    def test_complex_addition(self, input_mock, nums, expected, capsys):
    ...
    def test_complex_subtraction(self, input_mock, nums, expected, capsys):
        assert complex_arithmetic() == expected

    @pytest.mark.parametrize("nums, expected", [([1,2,3,4], "-5+ i10")])
    @patch('builtins.input', side_effect=["3", "1 2 3 4"])
    def test_complex_multiplication(self, input_mock, nums, expected, capsys):
        assert complex_arithmetic() == expected

    @pytest.mark.parametrize("nums, expected", [([1,2,3,4], "0.44+ i0.08")])
    @patch('builtins.input', side_effect=["4", "1 2 3 4"])
    def test_complex_division(self, input_mock, nums, expected, capsys):
        assert complex_arithmetic() == expected

    @patch('builtins.input', side_effect=["10"])
    def test_invalid_choice(self, input_mock, capsys):
        with pytest.raises(SystemExit):
            complex_arithmetic()

    @patch('builtins.input', side_effect=["1", ""])
    def test_empty_input(self, input_mock, capsys):
        with pytest.raises(SystemExit):
            complex_arithmetic()
