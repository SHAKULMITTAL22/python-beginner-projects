...
# ********RoostGPT********
import pytest
from math import factorial  # Using built-in factorial function for reliability and efficiency
...
# 1. Imported the built-in factorial function from the math module for better reliability and performance.
# 2. Removed custom factorial function implementation to avoid errors and improve efficiency.
# 3. This change ensures that the factorial computation is accurate and optimized, leveraging Python’s built-in capabilities.
# 4. Added appropriate error handling for negative inputs as the built-in factorial function raises a ValueError for negative numbers.
# 5. Ensured that the test module and methods adhere to Python coding standards (PEP 8) for readability and maintainability.
